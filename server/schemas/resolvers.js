...
const resolvers = {
    Query: {
        ...
        ipProxyRotator: async () => {
            ...
            const { error, status, data } = await axios.get('https://sslproxies.org/');
            // if (error) {
            //     console.log('Error loading proxy: ', error);
            // }
            ...
            return [ipAddresses, portNumbers];
        },
        ...
        getUrlArray: async (parent, { date, proxy }) => {
            const day = date.slice(8, 10);
                ...
                try {
                    ...
                    if ($('ul:eq(-1)').length === 0) {
                        $('ul:eq(0) .list-item', data).each(function () {
                            ...
                            const customId = buildCustomId(headliner, date, venue)
            ...
            $('div .show-date', data).each((index, value) => {
                $(value).toArray().map($).map(x => {
                    ...
                    x.find('ul .showlist-item').toArray().map($).map(y => {
                        const venueLink = y.find('a:eq(0)').attr('href');
                        const artists = y.find('a:eq(0)').text();
                        const venue = y.find('a:eq(1)').text();
                        const venueAddress = y.find('a:eq(2)').text();
                        const mapLink = y.find('a:eq(2)').attr('href');
                        //the following 'headliner' block focuses on finding any instance of 'w/' within `unfiltered headliner` and replaces it with 'with'.  The `headliner` variable is used in the customId which becomes a url for the event.  An `/` within the url causes an error. 
                        let headliner;
                        let unfilteredHeadliner = artists.split(',')[0];
                        const statusMatch = unfilteredHeadliner.match(/cancelled|sold\s?out/i);
